  ...
  fieldHashtag.addEventListener('input', function() {
    ...
    for(let i = 0; i < inputArray.length; i++){
      if(inputValue[0] !== '#')  {
        ...
        fieldHashtag.setCustomValidity('Хештег не может состоять только из одного символа #');
      } else if (inputArray[i].length > HASHTAGS_LENGTH) {
        fieldHashtag.setCustomValidity('Хэштег должен быть не больше 20 символов');
      ...
      } else if(!inputArray[i].endWidth(' ')){
        fieldHashtag.setCustomValidity('Должен быть пробел между хэштэгами')
...
export {validateHashtags, validComment};
