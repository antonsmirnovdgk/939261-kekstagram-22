...
// const isHashtagLetter = function(hashtag) {
...
const validateHashtags = function() {
  ...
  fieldHashtag.addEventListener('input', function() {
    ...
    for(let i = 0; i < inputArray.length; i++){
      ...
      if(inputArray[i][1] === ' ') {
        fieldHashtag.setCustomValidity('Хештег не может состоять только из символа #');
      }

      // if(inputArray[i][1] === '#') {
      //   fieldHashtag.setCustomValidity('Хештег только начинается с #');
      // }

      // if(isHashtagSymbol(inputArray[i][1])) {
      //   fieldHashtag.setCustomValidity('После # должны идти только буквы');
      // }

      // if(isHashtagEmoji(inputArray[i][1])) {
      //   fieldHashtag.setCustomValidity('Эмодзи использовать нельзя');
      // }

      // if(inputArray[i].length > HASHTAGS_LENGTH) {
      //   fieldHashtag.setCustomValidity('Хэштег должен быть не больше 20 символов'); //MAX длина одного хэштэга
      // }

      // if(inputArray[i] > MAX_HASHTAGS){
      //   fieldHashtag.setCustomValidity('Хэштег должен быть не больше 20 символов'); // MAX кол-во хэштегов
      // }

      // if(!isHashtagLetter(inputArray[i])){
      //   fieldHashtag.setCustomValidity('Хэштег может содержать как строчные так и прописные символы'); // Без повторений
      // }

      // if(inputArray[i].include(inputArray[i])){
